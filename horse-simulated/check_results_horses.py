...
def get_unsettled_bets():
    ...
    cursor.execute("""
        SELECT 
            ...
            horse_name, barrier_number, position_in_market, odds, stake, liability
        FROM paper_trades
...
def check_results():
    ...
    for market_id, market_bets in bets_by_market.items():
        ...
        # Check if settled - must have at least one WINNER
        ...
        for bet in market_bets:
            ...
            # Get ACTUAL BSP from settled market results (not projections)
            bsp_value = None
            if horse_result.get('bsp') is not None:
                bsp_value = horse_result.get('bsp')  # Already in results from API
            ...
            update_bet_result(bet_id, result, finishing_position, pnl, bsp_value)
...
def show_daily_stats():
    ...
    if rows:
        print("\n" + "="*100)
        print(f"📅 DAILY BREAKDOWN - HORSE LAY BETTING")
        print("="*100)
...
def export_to_csv():
    """Export all horse betting data to CSV files with 2 decimal places"""
    import os
    ...
    # date_str = "2026-01-15"  # Uncomment and set date for historical results
    date_str = datetime.now().strftime("%Y-%m-%d")  # Today's results
    ...
    cursor.execute("""
        SELECT 
            id, date, venue, country, race_number, market_id, selection_id,
            horse_name, barrier_number, position_in_market, odds, stake, liability,
            result, finishing_position, profit_loss, bsp
        FROM paper_trades
        WHERE date = ?
        ORDER BY id DESC
    """, (date_str,))
    
    csv_file = f"{results_dir}/horses_all_bets_{date_str}.csv"
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['ID', 'Date', 'Venue', 'Country', 'Race', 'Market ID', 'Selection ID',
                        'Horse Name', 'Barrier', 'Position', 'Odds', 'Stake', 'Liability',
                        'Result', 'Finish Pos', 'P&L', 'BSP'])
    ...
    csv_file = f"{results_dir}/horses_by_position_{date_str}.csv"
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
    ...
    csv_file = f"{results_dir}/horses_by_day_{date_str}.csv"
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
    ...
    csv_file = f"{results_dir}/horses_by_day_and_position_{date_str}.csv"
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
