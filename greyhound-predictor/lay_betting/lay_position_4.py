...
# Configuration
POSITION_TO_LAY = 4  # Laying the FAVORITE
MAX_ODDS = 500
...
class GreyhoundLayBetting:
    ...
    def get_upcoming_races(self) -> List[Dict]:
        ...
        try:
            ...
            conn = sqlite3.connect(RACE_TIMES_DB)
            query = """
            ...
            logger.debug(f"Found {len(df)} total races in database")
    ...
    def get_odds_and_runners(self, market_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Get runner names from Market Catalogue API
            cat_url = f"{BACKEND_URL}/api/market-catalogue/{market_id}"
            cat_response = self.session.get(cat_url, timeout=10)
            
            runner_names = {}
            if cat_response.status_code == 200:
                cat_data = cat_response.json()
                for runner in cat_data.get('runners', []):
            ...
            # Get names and box numbers from GreyhoundMarketBook table
            conn = sqlite3.connect(BETFAIR_DB)
            cursor = conn.cursor()
            cursor.execute("""
