#!/usr/bin/env python3
"""
Greyhound Simulation Results Analyzer
Integrates with Betfair Results Fetcher to calculate performance metrics for greyhound races
"""
...
class SimulationResultsAnalyzer:
    ...
    def get_simulation_bets(self) -> pd.DataFrame:
        ...
        query = """
            SELECT 
                ...
                bet_type,
                created_at
            FROM valid_greyhound_simulation_results
            WHERE race_date = ?
    ...
    def export_strategy_summary_csv(self, output_file: str = None):
        if output_file is None:
            ...
            output_file = f"greyhound_simulation_strategy_summary_{today}.csv"
    ...
    def export_performance_csv(self, output_file: str = None):
        if output_file is None:
            # Use current date for filename
            today = datetime.now().strftime("%d_%m")
            output_file = f"greyhound_simulation_performance_{today}.csv"
    ...
    def export_detailed_strategy_summary_csv(self, output_file: str = None):
        if output_file is None:
            # Use current date for filename
            today = datetime.now().strftime("%d_%m")
            output_file = f"greyhound_simulation_detailed_strategy_summary_{today}.csv"
        ...
        weighted_summaries = []
        for base_strategy in outcomes_df['base_strategy'].unique():
