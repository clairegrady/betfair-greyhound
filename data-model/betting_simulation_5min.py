...
8. Lay $1 on every horse at best lay odds
9. Proportional lay betting: $1 on longest odds, proportional stakes on others

Uses HTTP refresh approach (no Stream API) and processes races within 1 minute of start.
"""
...
class BettingSimulation:
    ...
    def init_database(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS valid_5min_simulation_results (
        ...
        try:
            cursor.execute('''
                ...
                ON valid_5min_simulation_results (race_date, venue, race_number, strategy, horse_name, bet_type)
            ''')
        except sqlite3.OperationalError:
            # Index might already exist, ignore
            pass
        
        # Create race tracking table for 5-minute simulation
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS race_tracking_5min_5min (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    def get_current_odds(self, market_id):
        ...
        try:
            ...
            # Get best back and lay odds (for both back and lay betting)
            current_odds_query = """
            SELECT 
                SelectionId,
    ...
    def simulate_strategy_6(self, horses, market_id, venue, race_number):
        ...
        bets.append({
            ...
            'horse_name': horses[0]['horse_name'],
        ...
        if horses[1]['place_odds'] > 0:
            bets.append({
                ...
                'horse_name': horses[1]['horse_name'],
        ...
        if horses[2]['place_odds'] > 0:
            bets.append({
                ...
                'horse_name': horses[2]['horse_name'],
    ...
    def save_simulation_results(self, bets, venue, race_number, market_id):
        ...
        for bet in bets:
            cursor.execute('''
                INSERT OR IGNORE INTO valid_5min_simulation_results 
    ...
    def is_race_already_processed(self, venue, race_number, race_date):
        ...
        try:
            ...
            cursor.execute('PRAGMA table_info(race_tracking_5min)')
            columns = [col[1] for col in cursor.fetchall()]
            
            if 'race_date' in columns:
                # Use race_date column
                cursor.execute('''
                    SELECT COUNT(*) FROM race_tracking_5min 
    ...
    def mark_race_as_processed(self, venue, race_number, race_date, market_id, bets):
        """Mark race as processed in race_tracking_5min table"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            if 'race_date' in columns:
                # Use race_date column
                cursor.execute('''
                    INSERT OR REPLACE INTO race_tracking_5min 
    ...
    def run_simulation(self):
        ...
        logger.info("🚀 Starting 5-minute betting simulation...")
        ...
        while True:
            ...
            for race in races:
                ...
                try:
                    ...
                    if -1 <= time_diff <= 5:  # Within 5-minute betting window (1 minute before to 5 minutes after)
                        if time_diff > 0:
                            logger.info(f"⏰ Race {venue} R{race_number} starts in {time_diff:.1f} minutes")
                            logger.info(f"🎯 Simulating 5-minute bets for {venue} R{race_number} (within 5 minutes of start)")
                        else:
