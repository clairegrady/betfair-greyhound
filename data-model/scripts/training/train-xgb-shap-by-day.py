...
# Dicts to collect SHAP values and feature samples by year
yearly_shap_values = {}
yearly_X_samples = {}
all_shap_values = []
all_X_samples = []
...
# Track which years have already been processed (SHAP plot exists)
processed_years = set()
...
# Aggregate results by year and overall
yearly_shap_values = {}
yearly_X_samples = {}
all_shap_values = []
all_X_samples = []
...
# Also aggregate all days combined
print("\nAggregating SHAP values across all days...")
...
try:
    ...
    # Save SHAP summary plot
    plot_path = shap_plot_dir / "shap_summary_plot_aggregated_all_days.png"
    plt.figure()
    shap.summary_plot(shap_all, X_all, show=False)
    plt.savefig(plot_path)
    plt.close()
    print("Aggregated SHAP summary plot saved as shap_summary_plot_aggregated_all_days.png")
    # Save mean absolute SHAP values (feature importance) as CSV
    ...
    csv_path = shap_plot_dir / "shap_feature_importance_aggregated_all_days.csv"
    importance_df.to_csv(csv_path, index=False)
    print("Saved SHAP feature importance CSV for all days")
    # Upload to S3 in an 'overall' subfolder
    s3_plot_path = f"{s3_bucket}/{s3_output_prefix}overall/shap_summary_plot_aggregated_all_days.png"
    s3_csv_path = f"{s3_bucket}/{s3_output_prefix}overall/shap_feature_importance_aggregated_all_days.csv"
    s3.put(plot_path.as_posix(), s3_plot_path)
    s3.put(csv_path.as_posix(), s3_csv_path)
    print("Uploaded overall SHAP plot and CSV to S3 subfolder 'overall'.")
except Exception as e:
    print(f"❌ Could not aggregate SHAP values or save combined plot: {e}")
    with skipped_log.open("a") as f:
        f.write(f"AGGREGATION: {e}\n")
